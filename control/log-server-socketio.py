			...
			if portbusy == False:
				...
				if feedback: ##rear HB
					if feedback[0] == 205 and feedback[1] == 171: #check start byte
						...
						mqttclient.publish("r_cmd1", cmd1)
						mqttclient.publish("r_cmd2", cmd2)
						mqttclient.publish("r_speedR_meas", speedR_meas)
						mqttclient.publish("r_speedL_meas", -speedL_meas)
						mqttclient.publish("r_batVoltage", batVoltage/100)
						mqttclient.publish("r_dcCurrent", dcCurrent/100)
						mqttclient.publish("r_boardTemp", boardTemp/10)
						mqttclient.publish("r_cmdLed", cmdLed)
				...
				if fourwd == True:
					...
					if feedback2: ##front HB
						if feedback2[0] == 205 and feedback2[1] == 171: #check start byte
							...
							mqttclient.publish("f_cmd1", cmd1)
							mqttclient.publish("f_cmd2", cmd2)
							mqttclient.publish("f_speedR_meas", speedR_meas)
							mqttclient.publish("f_speedL_meas", -speedL_meas)
							mqttclient.publish("f_batVoltage", batVoltage/100)
							mqttclient.publish("f_dcCurrent", dcCurrent/100)
							mqttclient.publish("f_boardTemp", boardTemp/10)
							mqttclient.publish("f_cmdLed", cmdLed)
